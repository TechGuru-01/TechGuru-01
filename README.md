# Elijah Boon

**Computer Engineer — AI, Computer Vision & Full-Stack Development**

I am an aspiring Software Developer dedicated to solving real-world problems with production-ready software that connects artificial intelligence, computer vision, and modern web technologies. My work spans responsive web applications, scalable backend APIs, AI-powered systems, and embedded IoT devices. I focus on clean architecture, automation, and practical machine learning to solve real-world problems.

---

## What I Do

- Build AI and computer vision applications
- Develop full-stack web applications
- Design and scale backend APIs and services
- Integrate software with IoT and embedded devices
- Containerize and deploy applications using Docker

---

## Currently Seeking

I am actively looking for opportunities in the following roles:

- Computer Vision Engineer
- Python Developer
- Backend Developer
- Full Stack Developer
- Software Engineer

Open to remote, hybrid, and on-site positions.

---

## Technical Skills

**Languages:** Python, Java, JavaScript, PHP, C++, C, HTML, CSS

**Frontend:** React, Vite, HTML, CSS, JavaScript

**Backend:** Node.js, Express, FastAPI, Flask, PHP

**AI / Machine Learning / Computer Vision:** PyTorch, OpenCV, NumPy, Pandas, Scikit-learn, Matplotlib, SciPy, Jupyter, CUDA

**Databases:** MySQL, PostgreSQL, SQLite, Firebase, Supabase

**DevOps & Tools:** Docker, Git, GitHub, GitHub Actions, Vercel, Netlify, Postman, VS Code, CMake

**Embedded Systems:** Arduino, Raspberry Pi, Jetson Orin Nano

**AI Tools:** ChatGpt, Gemeni, Claude, AI Studio

---

## Contact

- **LinkedIn:** [linkedin.com/in/Elijah-Boon](linkedin.com/in/elijah-boon-6000383ab)
- **Email:** [elijahboon986@gmail.com](mailto:elijahboon986@gmail.com)

If you're looking for someone to build AI applications, backend systems, computer vision solutions, or full-stack software, I'd welcome the opportunity to connect.

---

*Building intelligent software that bridges AI, cloud, and embedded systems.*
